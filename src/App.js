...
function App() {
  ...
  function getDemands() {
    axios.get(`${URL}demands/testuser1`).then(function (response) {
      //console.log("DEMANDS DATA"+JSON.stringify(response.data,0,2));
      setDemands(response.data);
    });
  }
  function getBenchResources() {
    axios.get(`${URL}bench/testuser1`).then(function (response) {
...
<h1>DXC Talent Pool</h1>
</div>
<div className='name'>Welcome! Shweta </div>
<div className="row">
  <div className="side">
      ...
      <ul>“The person born with a talent they are meant to use will find their greatest happiness in using it. ”</ul>
     
      </div>
      <div className='idea'>This tool provides an effective mapping of people available on bench to the current demands on click of a button</div>
  </div>
  <div className="main">
  <img height="100%" width="100%" src= {Img} alt="pic" />
    ...
    <div className='future'>Future Expansion</div>
